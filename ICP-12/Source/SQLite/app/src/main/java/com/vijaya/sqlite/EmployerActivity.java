...
public class EmployerActivity extends AppCompatActivity {
    ...
    private void readFromDB() {
        ...
        SampleRecyclerViewCursorAdapter adaptor = new SampleRecyclerViewCursorAdapter(this, cursor,
                new SampleRecyclerViewCursorAdapter.AdaptorListener() {         // implement listener
                    @Override
    ...
    private void deleteFromDB(String name) {
        ...
        if (database.delete(SampleDBContract.Employer.TABLE_NAME , SampleDBContract
                .Employer.COLUMN_NAME + "=?" , new String[ ]   { name } ) > 0) {               // match name to delete record
            Toast.makeText(this, "Record \"" + name + "\" deleted.", Toast.LENGTH_LONG).show();
        }
        readFromDB();               // update DB display
    }
}