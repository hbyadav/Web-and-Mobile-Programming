...
public class EmployeeActivity extends AppCompatActivity {
    ...
    private void readFromDB() {
        ...
        SampleJoinRecyclerViewCursorAdapter adaptor = new SampleJoinRecyclerViewCursorAdapter(this, cursor,
                new SampleJoinRecyclerViewCursorAdapter.AdaptorListener() {         // implement listener
                    @Override
                    public void deleteButtonOnClick(View v, String name) {
                        Log.d(TAG, "Delete listener on record name "+ name + " called");
                        deleteFromDB(name);                                         // call delete method
                        Log.i(TAG, "Record \"" + name + "\" deleted");
                    }

                    @Override
                    public void updateButtonOnClick(View v, String fn, String ln, String jdesc, String dob) {
                        Log.d(TAG, "Update listener on record name  "+ fn + " called");
                        updateDB(fn, ln, jdesc, dob);                             // call update method
                    }
                });
        binding.recycleView.setAdapter(adaptor);
    }
    private void deleteFromDB(String name) {
        SQLiteDatabase database = new SampleDBSQLiteHelper(this).getWritableDatabase();     // get DB instance
        if (database.delete(SampleDBContract.Employee.TABLE_NAME , SampleDBContract
                .Employee.COLUMN_FIRSTNAME + "=?" , new String[ ]   { name } ) > 0) {               // match name to delete record
            Toast.makeText(this, "Record \"" + name + "\" deleted.", Toast.LENGTH_LONG).show();
        }
        readFromDB();               // update DB display after deleting
    }

    private void updateDB(String fn, String ln, String jdesc, String dob) {
                // create intent to go to update activity
        Intent toUpdate = new Intent(EmployeeActivity.this, UpdateActivity.class);
                // pass current employee values to activity
        toUpdate.putExtra("fName", fn);
        toUpdate.putExtra("lName", ln);
        toUpdate.putExtra("jDesc", jdesc);
        toUpdate.putExtra("dob", dob);
